'''
Financial Assistant Application | ITERATION 2
George R
July 2024
'''
from tkinter import * 
...
def home():
    ...
    logo = Image.open("iteration 2/diplogo.png") # Load the logo image
    ...
    image = Image.open("iteration 2/spend.jpeg") 
    ...
    image2 = Image.open("iteration 2/school.jpg") 
    ...
    image3 = Image.open("iteration 2/a.jpeg") 
...
class SpendTracker:
    ...
    def add_item(self):
        ...
        frame1 = ttk.Frame(root, padding=10) # Frame for item entry
        frame1.grid(row=0, column=0, columnspan=4, sticky="NSEW")
        
        # Entry for item name
        ttk.Label(frame1, text="Item Name:").grid(row=0, column=0, padx=5, pady=5, sticky=E)
        self.p_name = ttk.Entry(frame1, width=15)
        self.p_name.grid(row=0, column=1, padx=5, pady=5)
        
        # Dropdown for category
        ttk.Label(frame1, text="Category:").grid(row=0, column=2, padx=5, pady=5, sticky=E)
        self.p_cat = ttk.Combobox(frame1, values=self.cat_list, width=15)
        self.p_cat.grid(row=0, column=3, padx=5, pady=5)
        
        # Spinbox for price
        ttk.Label(frame1, text="Price:").grid(row=1, column=0, padx=5, pady=5, sticky=E)
        ...
        # Calendar for date selection - this uses the imported module from top
        ttk.Label(root, text="Select Date:").grid(row=1, column=0, sticky=W)
        self.p_date = Calendar(root, selectmode="day", year=2024, foreground="black", selectforeground="blue")
        self.p_date.grid(row=2, column=0)
        
        # Text area for displaying spending history
        ttk.Label(root, text="Spending History:").grid(row=3, column=0, pady=5, sticky='nesw')
        self.display_area = Text(root, height=10, width=50)
    ...
    def save_items(self, item):  
        with open("iteration 2/spending.txt", "a") as file:  # Append new item to text file
    ...
    def display_text(self):
        ...
        with open("iteration 2/spending.txt", "r") as file:
...
def ai_chat():
    ...
    os.environ["GENERATIVE_AI_KEY"] = "XXXXXXXXXXXXXXXXXX" # API key
...
def school_frame():
    ...
    def show_content(topic):
        ...
        if topic == "Learn about budgeting":
            file_path = "iteration 2/scl1.txt"
        elif topic == "When to save/spend":
            file_path = "iteration 2/scl2.txt"
        elif topic == "What is investing?":
            file_path = "iteration 2/scl3.txt"
        elif topic == "How to track spending?":
            file_path = "iteration 2/scl4.txt"
        elif topic == "Summary":
            file_path = "iteration 2/scl5.txt"
