'''
Financial Assistant Application | ITERATION 3
George R
AUG 2024
'''
...
def load_users(): # function to open and read the jason file for passwords
    with open("iteration 3/users.json", "r") as file:
        return json.load(file)

def save_users(users):# function to open and write the jason file for passwords
    with open("iteration 3/users.json", "w") as file:
        json.dump(users, file) # writing the username and password to the json

def login(): # log in function
    username = username_entry.get() # changes the custom tkinter data into a string, usable to read
    password = password_entry.get()#  changes the custom tkinter data into a string, usable to rad
    users = load_users() # sets users var as the function
    
    if users.get(username) == password:# if the password is valid and is correct
        login_frame.grid_forget()# hide the log in frame
        show_spend_tracker() # and show the spending tracker frame
    else:
...
def create_account(): # if the user selects create account, this will run
    new_username = new_username_entry.get()# changes the custom tkinter data into a string, usable to store 
    new_password = new_password_entry.get()# changes the custom tkinter data into a string, usable to store 
    users = load_users()

    if not new_username or not new_password: # if the user enters an unvalid username or password in the creat account function
        create_account_error_label.configure(text="Please enter both username and password.") # if invalid, display error message
        return # return to creat account function

    if new_username in users:
        create_account_error_label.configure(text="Username already exists. Please choose another.")# if the user enters a username that already exists, it will display this message
        return# return to creat account function

    users[new_username] = new_password  # Store the new username and password in the users dictionary
    save_users(users)  # Save the updated users dictionary

    create_account_content.grid_forget()  # Hide the account creation content
    login_content.grid(row=1, column=0, sticky="ns")  # Show the login content and positioning it in the grid

    error_label.configure(text="Account created successfully. Please log in.")  # Update label to say that account was created successfully 
...
image = ctk.CTkImage(Image.open("iteration 3/diplogo.png"), size=(800, 342))
...
def create_top_bar(parent):  # setting up top bar 
    ...
    logo = ctk.CTkImage(Image.open("iteration 3/diplogo.png"), size=(140, 60))  # Loading the logo image to be displayed on the top bar
...
class SpendTracker:
    ...
    def save_items(self, item):
        ...
        with open("iteration 3/spending.txt", "a") as file:
    ...
    def display_text(self):
        ...
        with open("iteration 3/spending.txt", "r") as file:
            content = file.read()
...
def ai_chat(parent_frame):
    ...
    os.environ["GENERATIVE_AI_KEY"] = "XXXXXXXXXXXXXXX"  
...
def school_frame(parent_frame):
    ...
    def show_content(topic):
        ...
        file_path = "iteration 3/scl1.txt"
        try:
