'''
Financial Assistant Application | ITERATION 5
George R
SEP 2024
'''
...
def load_users():# Load users from a json file
    with open("iteration 5/users.json", "r") as file:
        return json.load(file)

def save_users(users):# save user data back into the json file
    with open("iteration 5/users.json", "w") as file:
...
def play_video_in_image_frame():# Function to play the video in the left hand rectangle within the log in and create account page
    ...
    video_path = os.path.join(os.getcwd(), "iteration 5/better.mp4") #  path to video file
...
def create_account():# handle the account creation process
    ...
    if not new_username or not new_password: # make sure that both username and password are provided
        create_account_error_label.configure(text="Please enter both username and password.") # configure the text so that this error appears if they only enter one line
        return # return to function

    if new_username in users:# Check if the username already exists
        create_account_error_label.configure(text="Username already exists. Please choose another.") # as above, error comes up if the user enters a username that already exists in the database
        return

    try:# make sure that the salary input is numeric
        salary = float(salary) if salary else 0  # store salary if provided, otherwise default to 0
    except ValueError:
        create_account_error_label.configure(text="Please enter a valid numeric salary.")
        return
    # Save the user all of the data to the json
    users[new_username] = {"password": new_password, "spending_history": [], "saving_goal": 0, "progress": 0, "salary": salary } # Use the validated salary
    save_users(users)

    create_account_content.grid_forget() # once the account has been fcreated, it forgets the screen and then it shows the log in screen
    login_content.grid(row=1, column=0, sticky="ns")
    success_label.configure(text="Account created successfully. Please log in.") # account success text 
...
# New Username and password entry fields for create account screen
...
# Button to actiate for tbe new account
...
def settings():
    ...
    # label for feedback on salary actions
...
def create_top_bar(parent):
    ...
    logo = ctk.CTkImage(Image.open("iteration 5/diplogo.png"), size=(150, 64))
...
def create_sidebar(parent):# sidebar func for navigation 
    ...
    set_button_image = ctk.CTkImage(Image.open("iteration 5/settings.png"), size=(140, 99))# settings button with an image as the button.
...
class AIChat:
    ...
    def create_ai_chat_frame(self):
        ...
        os.environ["GENERATIVE_AI_KEY"] = "XXXXXXXXXXXXXXXXX"  # google gemini key
...
class SalaryTracker: # SalaryTracker class for the frame to show the income of the user
    ...
    def create_salary_tracker_frame(self):
        ...
        ctk.CTkLabel(frame, text="Projected Salary Over 5 Years", font=("Arial", 18)).pack(pady=10)      # title
        ...
        ax.set_title("Salary Growth Over 5 Years")
        ax.set_xlabel("Years")
...
class SchoolFrame:# class for the finance school interface
    ...
    def show_content(self, topic): # function to show the content based on what the user asked for
        ...
        file_path = "iteration 5/scl1.txt" # importing the text file that has the informaiton
        try:
